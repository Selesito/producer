...
class ProducerKafkaResourceIT {
    ...
    void setup() {
        ...
        ProducerKafkaResource kafkaResource = new ProducerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/producer-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/producer-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
